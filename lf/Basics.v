...
Example true_and_false: (true && false) = false.
Proof. simpl. reflexivity. Qed.

Example false_and_false: (false && false) = false.
Proof. simpl. reflexivity. Qed.
...
Fixpoint bin_to_nat_helper (m:bin) (d:nat) : nat :=
  match m with
  | Z => O
  | B0 m' => bin_to_nat_helper m' (S d)
  | B1 m' => (exp 2 d) + (bin_to_nat_helper m' (S d))
  end.
  

Fixpoint bin_to_nat (m:bin) : nat :=
  bin_to_nat_helper m O.
